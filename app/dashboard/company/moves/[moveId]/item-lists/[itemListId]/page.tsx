...
type PackingStatus = 'UNPACKED' | 'PACKED' | 'LOADED' | 'UNLOADED' | 'UNPACKED_AT_DESTINATION';
...
export default function ItemListPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await fetch(`/api/company/moves/${moveId}/item-lists/${itemListId}/items`)
  ...
  const handleEdit = (itemId: string) => {
    router.push(`/dashboard/company/moves/${moveId}/item-lists/${itemListId}/items/${itemId}/edit`)
  }
  ...
  const handleItemClick = (itemId: string) => {
    router.push(`/dashboard/company/moves/${moveId}/item-lists/${itemListId}/items/${itemId}`)
  }
  ...
  return (
    <div className="container py-10">
      <div className="flex justify-between items-center mb-8">
        ...
        <Button asChild>
          <Link href={`/dashboard/company/moves/${moveId}/item-lists/${itemListId}/items/create`}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => {
          const StatusIcon = STATUS_CONFIG[item.packingStatus]?.icon || Package
          const statusConfig = STATUS_CONFIG[item.packingStatus] || STATUS_CONFIG.UNPACKED
          return (
            <Card
              key={item.id}
              className={cn(
